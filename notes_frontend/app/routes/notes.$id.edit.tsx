...
import { apiGetNote, apiUpdateNote, type Note } from "~/utils/api.server";
import { requireTokenOrRedirect } from "~/utils/session.server";

// PUBLIC_INTERFACE
export async function loader({ request, params }: LoaderFunctionArgs) {
  /**
   * Ensure authenticated and load note by :id.
   */
  const token = await requireTokenOrRedirect(request);
  const id = Number(params.id);
  if (!Number.isFinite(id)) throw redirect("/notes");
  try {
    const note = await apiGetNote(token, id);
    return json({ note });
  } catch (e: unknown) {
    const status = (e as { status?: number } | null | undefined)?.status;
    if (status === 404) return redirect("/notes");
    throw e;
  }
}
...
export async function action({ request, params }: ActionFunctionArgs) {
  ...
  const token = await requireTokenOrRedirect(request);
  const id = Number(params.id);
  if (!Number.isFinite(id)) return redirect("/notes");
  ...
  try {
    await apiUpdateNote(token, id, { title, content, tags });
    return redirect("/notes");
  } catch (e: unknown) {
    const status = (e as { status?: number } | null | undefined)?.status ?? 500;
    return json({ error: "Failed to update note." }, { status });
  }
}
